...
The rewrite touched these tests in `tests/velocityVaults.ts`:
...
**Risk:** any `velocity_vaults` code path that's specific to summing two positive
...
velocity's `handle_begin_swap` / `handle_end_swap`. On closer inspection that
path is closed:

- `velocity_vaults` has **no swap instruction wrapper** — none of the 49
  instructions in `programs/velocity_vaults/src/instructions/` CPI into velocity's
  swap pair, and `velocity_cpi.rs` has no swap accounts struct. Adding one is a
  ~300-line production feature change (new instruction file, lib.rs entry,
  velocity_cpi wrapper, vaultClient TS method, IDL regen, plus security review of
  manager-controlled swap surface) — well beyond the scope of test coverage
  backfill.
- Calling velocity's swap directly from `managerVelocityClient` (the vault's
  delegate) does work as far as `can_sign_for_user` is concerned
  (`programs/velocity/src/instructions/constraints.rs:18`), but velocity's swap
  ...
  rejected (`programs/velocity/src/instructions/user.rs:3624` —
  ...
  velocity-side `tests/spotSwap.ts` reference is unavailable to a vault context;
  it would require wiring openbook (or another whitelisted DEX) into the
  velocity-vaults test fixtures.
- `manager_borrow` (the closest existing instruction that touches a
  non-`vault.spot_market_index` market) enforces
  `velocity_spot_market_vault.mint == vault_token_account.mint`
  (`programs/velocity_vaults/src/instructions/manager_borrow.rs:75-79`), so it
...
1. *Rust unit test in `programs/velocity_vaults/src/tests.rs`* — construct a
...
3. *Add a `swap` wrapper to velocity-vaults* — a real production feature.
...
intermediate `apply_rebase` firing during the velocity, fee/share interactions
...
`programs/velocity_vaults/src/tests.rs` cover share-math edges per iteration so
the math itself is exercised — but the *integration* loop (velocity CPI →
velocity_vaults CPI → vault state mutation, repeated) is not.
...
`TestTokenizedVelocityVaults` (commented at the call site). It opens a 1x
...
because velocity's maintenance-margin auto-liquidation and
...
Why skipped: the test trips velocity's
`validate_fill_price_within_price_bands` (`programs/velocity/src/math/orders.rs:511`)
...
reasonable fill at the restored oracle diverges past velocity's 50% TWAP band
...
- The four rewritten tests are in `tests/velocityVaults.ts` (search for
...
- `programs/velocity_vaults/src/state/vault.rs:499` (`InvalidEquityValue`) is the
  guard that vetoes negative `vault_equity`. Velocity's `vault_equity` is `i128`
  (see `calculate_user_equity` in `programs/velocity/src/math/margin.rs`); going
...
- For future debugging, the loop in `doWashTrading` swallows errors via
  `assert(false, ...)` only when `i < 5`. To trace what velocity is actually
