...
The codebase has been migrated from Anchor 0.29 to 1.0, and fully rebranded drift → velocity (mirroring shadow's own rebrand). The `velocity` crate is consumed as a **local path dep** at `../protocol-v2-shadow/programs/velocity` (see `programs/velocity_vaults/Cargo.toml`). `velocity.so` / `pyth.so` fixtures under `tests/fixtures/` were rebuilt from that same shadow repo — they MUST be regenerated from shadow whenever you pull shadow changes (`bun run program:build` in shadow uses the right feature set: `--no-default-features --features no-entrypoint,anchor-test`; copy `target/deploy/velocity.so` + `target/idl/velocity.json` into `tests/fixtures/`), or the local validator tests will fail on deserialization. The TS dep is the published `@velocity-exchange/sdk` (pinned in `ts/sdk/package.json`). Once shadow publishes a matching crates.io release, the Rust path dep can be flipped back too.
