...
const program = anchor.workspace.VelocityVaults as Program<VelocityVaults>;
...
mockUSDCMint(provider, usdcMint)
	.then(async () => {
		if (adminClient && (await isVelocityInitialized(adminClient))) {
			console.log('Velocity already initialized');
			return;
		}
...
describe('TestProtocolVaults', () => {
	...
	it('Long SOL-PERP', async () => {
		...
		const vaultUserAcct = (
			await delegateClient.velocityClient.getUserAccountsForDelegate(
				delegate.publicKey
			)
		)[0];
		...
		try {
			...
			const placeAndMakeOrderIx = await (
				delegateClient.velocityClient.program.methods as any
			)
	...
	it('Settle Pnl', async () => {
		...
		const solPrice = vaultUser.velocityClient.getOracleDataForPerpMarket(0);
		console.log(
			'SOL price:',
...
describe('TestTokenizedVelocityVaults', () => {
	const bulkAccountLoader = new BulkAccountLoader(connection, 'confirmed', 1);
	...
	async function testRedeemVaultTokensWithProfitShare({
		...
		const solSpotMarket = managerVelocityClient.getSpotMarketAccount(1);
		...
		const [velocityClient, usdcAccount, kp] = await createUserWithUSDCAccount(
			provider,
			usdcMint,
			new anchor.Program(
				managerVelocityClient.program.idl as anchor.Idl,
		...
		try {
			await depositorVaultClient.deposit(
				...
				{
					vault,
					authority: depositorVaultClient.velocityClient.wallet.publicKey,
				},
				{ noLut: true },
				usdcAccount
			);
		...
		const delegateVelocityClient = new VelocityClient({
		...
		try {
			await delegateVelocityClient.placeAndTakePerpOrder(
				{
		...
		const solPrice1 =
			delegateVelocityClient.getOracleDataForPerpMarket(0).price;
		const vaultEquity2 =
			await depositorVaultClient.calculateVaultEquityInDepositAsset({
				address: vault,
			});
		...
		await mmVelocityClient.unsubscribe();
	...
	it('Disallow tokenize after vault rebases, allow redeeming tokens', async () => {
		...
		await vd0Client.deposit(
			...
			{
				vault,
				authority: vd0VelocityClient.wallet.publicKey,
			},
			{ noLut: true },
			vd0UsdcAccount
		);
		...
		await managerVelocityClient.addAndSubscribeToUsers(vault);
		await managerVelocityClient.switchActiveUser(0, vault);
		...
		// Pure perp wash trading can't get there on its own because velocity's AMM provides tight
		...
		const oracleBeforeCrash =
			managerVelocityClient.getOracleDataForPerpMarket(0).price;
		await requoteFunc();
		const mmUserForCrash = mmVelocityClient.getUser();
		...
		const vaultUserForCrash = managerVelocityClient.getUser(0, vault);
		...
		//  • clearing velocity's strict margin check (≤ 5x max via marginRatioInitial=2000), and
		//  • keeping the oracle/twap ratio under velocity's default `too_volatile_ratio=5` so
		...
		await managerVelocityClient.placeAndTakePerpOrder(
			{
				orderType: OrderType.LIMIT,
				marketIndex: 0,
		...
		// equity in (0, ~$5] regardless of fill price (velocity's perp AMM mixes maker fill at
		...
		await managerVelocityClient.fetchAccounts();
		const refreshedVaultUser = managerVelocityClient.getUser(0, vault);
		const perpPos = refreshedVaultUser.getPerpPosition(0);
		// velocity stores quote_asset_amount as the position's quote balance (negative for longs that
		...
		await vd1Client.deposit(
			...
			{
				vault,
				authority: vd1VelocityClient.wallet.publicKey,
			},
			{ noLut: true },
			vd1UsdcAccount
		);
		...
		await vd1VelocityClient.sendTransaction(
			await vd1VelocityClient.buildTransaction(
				rebaseIx,
				vd1VelocityClient.txParams
			),
			[],
			vd1VelocityClient.opts
		);
		...
		await managerVelocityClient.sendTransaction(
			await managerVelocityClient.buildTransaction(
				vdtRebaseIx,
				managerVelocityClient.txParams
			),
			[],
			managerVelocityClient.opts
		);
		...
		try {
			...
			assert(
				(await vd0VelocityClient.connection.getAccountInfo(
	...
	// actually mutates between velocity state changes. Bias UP rather than DOWN so we don't
	// have to dance around velocity's maintenance-margin auto-liquidation or
	...
	// mutated only by velocity's internal AMM update paths over wall-clock minutes — not
	...
	// the restored oracle diverges past velocity's 50% TWAP band (orders.rs:511) and gets
	...
	it.skip('Vault profit share is consistent under gradual equity gain', async () => {
		...
		await vd0Client.deposit(
			vd0VaultDepositor,
			usdcAmount,
			{ vault, authority: vd0VelocityClient.wallet.publicKey },
			{ noLut: true },
			vd0UsdcAccount
		);
		...
		const vaultUser = managerVelocityClient.getUser(0, vault);
		...
		await doWashTrading({
			...
			midLoopHook: async () => {
				...
				await vd0VelocityClient.sendTransaction(
					await vd0VelocityClient.buildTransaction(
						ix,
						vd0VelocityClient.txParams
					),
					[],
					vd0VelocityClient.opts
				);
				profitShareCalls++;
				await validateTotalUserShares(program, vault);
...
describe('TestInsuranceFundStake', () => {
	...
	beforeAll(async () => {
		...
		bulkAccountLoader.startPolling();
		await bulkAccountLoader.load();
		await managerVelocityClient.subscribe();
		await vd0VelocityClient.subscribe();
	...
	const testInsuranceFundStake = async (marketIndex: number) => {
		...
		const ifStakeAccount = (await (
			managerVelocityClient.program as any
		...
		const managerTokenAccountBalance =
			await managerVelocityClient.connection.getTokenAccountBalance(
				managerTokenAccount
			);

		const vd0TokenAccountBalance =
			await vd0VelocityClient.connection.getTokenAccountBalance(
				vd0TokenAccount
			);
...
describe('TestSOLDenomindatedVault', () => {
	...
	it('Test deposit then withdraw SOL', async () => {
		const balanceBefore = await vd0VelocityClient.connection.getBalance(
			vd0Signer.publicKey
		);
		...
		const balanceAfter = await vd0VelocityClient.connection.getBalance(
			vd0Signer.publicKey
		);
		...
		const balanceEnd = await vd0VelocityClient.connection.getBalance(
			vd0Signer.publicKey
		);
		console.log(
...
describe('TestWithdrawFromVaults', () => {
	...
	it('Test manager cancel withdraw owning 100% of vault', async () => {
		...
		try {
			const oracle0 = mmVelocityClient.getOracleDataForPerpMarket(0);
			...
			await managerVelocityClient.addAndSubscribeToUsers(commonVaultKey);
